...
export proc binary(ctx: Context, name: Str, run_help: Bool) [fs, process, error, io] -> Result[Unit] {
  ...
  if ! product.exists()? {
    return Err(context.ContextError.StageFailed(stage: "verify-binary", target: ctx.target_triple, detail: f"missing artifact ${product.display()}"))
  }

  let metadata = product.metadata()?

  if metadata.size < 1024 {
    return Err(context.ContextError.StageFailed(stage: "verify-binary", target: ctx.target_triple, detail: f"implausibly small artifact ${product.display()}"))
  }

  if ! product.executable()? {
    return Err(context.ContextError.StageFailed(stage: "verify-binary", target: ctx.target_triple, detail: f"artifact is not executable ${product.display()}"))
  }
  ...
  if ctx.target_os == "linux" {
    ...
    if data.len() < 4 or data.slice(0, length: 4) != b"\x7fELF" {
      return Err(context.ContextError.StageFailed(stage: "verify-elf", target: ctx.target_triple, detail: f"not an ELF executable ${product.display()}"))
    }

    let header = run.capture --text readelf -h $product ?

    if ! header.status.ok or ctx.elf_machine not in header.stdout {
      return Err(context.ContextError.StageFailed(stage: "verify-elf-machine", target: ctx.target_triple, detail: f"wrong ELF machine for ${product.display()}"))
    }

    let dynamic = run.capture --text readelf -d $product ?

    if ! dynamic.status.ok or "NEEDED" in dynamic.stdout {
      return Err(context.ContextError.StageFailed(stage: "verify-static", target: ctx.target_triple, detail: f"dynamic dependency found in ${product.display()}"))
    }
  } else {
    ...
    if ! description.status.ok or ctx.executable_format not in description.stdout or "arm64" not in description.stdout {
      return Err(context.ContextError.StageFailed(stage: "verify-format", target: ctx.target_triple, detail: f"wrong executable format for ${product.display()}"))
    }
  }
