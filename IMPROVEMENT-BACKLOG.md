...
- **B11 · P1.** Finish feature/profile and copied-product gates, then close the migration. `results-b11-cumulative-macos-final.json` passes all 24 applicable workloads in three rounds with exact parity and six Linux-only skips. `results-b11-cumulative-linux.json` passes all 30 pinned Linux workloads in three rounds with exact parity. The earlier full run exposed `text_wrap_unicode`; its `Str.wrap` and `Str.fields` bindings returned to native, and `bench/stdlib-port/results-b11-text-wrap.json` records passing fixed workloads on both hosts. The first Linux cumulative round exposed `linux.meminfo`, `system.memory`, and `system.os_release`; `results-b11-linux-text-{prechange,native}.json` records three failing embedded and three passing native B0 workloads with exact parity. Their unused embedded sources and append bridge were removed. Final native corpus and stdlib port checks pass on both hosts; repeat the API checks after final builds.
