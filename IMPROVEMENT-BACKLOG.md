...
- **B11 · P1.** Finish the pinned Linux cumulative B0 and feature/profile gates, then close the migration only when both host reports pass. `results-b11-cumulative-macos.json` passes all 24 applicable workloads in three rounds with exact parity. The earlier full run exposed `text_wrap_unicode`; its `Str.wrap` and `Str.fields` bindings returned to native, unused embedded-method code was removed, and `bench/stdlib-port/results-b11-text-wrap.json` records passing fixed workloads on both hosts. Focused architecture, native corpus, and API gates pass after the final source change; repeat copied-product checks after final builds.
