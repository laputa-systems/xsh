...
- **A01 · P1.** Continue the native/Rust ownership audit in the remaining runtime groups. Module, frozen indexed, language diagnostic, and Unix dry-run assertions now live in `tests/xsh/`; Rust retains CLI, host fixture, process lifecycle, raw-byte, and allocation boundaries. Audit remaining assertion-only cases without replacing host fixtures.
