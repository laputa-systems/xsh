...
- **A01 · P1.** Continue the native/Rust ownership audit beyond the completed stream module, the `fs.walk` value checks in `tests/xsh/stdlib/fs.xsh`, explicit `main()` invocation in `tests/xsh/run.xsh`, loaded-module checks in `tests/xsh/stdlib/module.xsh`, and collection updates in `tests/xsh/stdlib/map.xsh`; move language assertions to disk-backed native tests when the host harness adds no boundary.
