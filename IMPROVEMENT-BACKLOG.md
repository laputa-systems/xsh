...
- **B11 · P1.** Finish the pinned Linux cumulative B0 and feature/profile gates, then close the migration only when both host reports pass. `results-b11-cumulative-macos.json` passes all 24 applicable workloads in three rounds with exact parity before the Linux reader cleanup. The earlier full run exposed `text_wrap_unicode`; its `Str.wrap` and `Str.fields` bindings returned to native, and `bench/stdlib-port/results-b11-text-wrap.json` records passing fixed workloads on both hosts. The first Linux cumulative round exposed `linux.meminfo`, `system.memory`, and `system.os_release`; `results-b11-linux-text-{prechange,native}.json` records three failing embedded and three passing native B0 workloads with exact parity. Their unused embedded sources and append bridge were removed. Repeat the complete host, copied-product, native corpus, API, and profile gates after final builds.
