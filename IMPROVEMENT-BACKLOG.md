...
- **A04 · P1/M.** Resolve the `reduce-by --jobs` execution mismatch and adjacent fusion cost. `FullStageTag::ReduceBy` is serial; an explicit `--jobs` expression runs once, validates a positive count, and prevents adjacent `par-map |> reduce-by` fusion. On `showcase/ecount.xsh` and `showcase/tokei.xsh`, adding `--jobs=8` showed no reliable gain. On an adjacent 50,000-row synthetic pipeline, the fused form took about 141 ms versus 47 ms with explicit `reduce-by --jobs=8`, with exact output parity. Keep this useful fusion escape hatch while isolating the fused path's cost on a representative workload; then decide whether to repair fusion or narrow the option contract. Do not add worker execution without deterministic error and effect semantics.
