...
- **J11 · P2.** Migrate the x86_64 installer image builder from removed `pm install`. `../packages` now writes a target-specific x86_64 BuildPlan; the 24-node installer selection produced byte-identical plans in two runs. Its executor, artifact receipts, root composition, and generation records preserve the target; a three-package x86_64 executor fixture passed, and the native PM gate passed with two host skips. `pm repo build` still rejects x86_64 before store mutation because no x86_64 runner is configured. Build and prove that runner against real x86_64 packages, then migrate the installer while preserving its installer and target roots. Run its image and QEMU tests; the published x86_64 kernel currently stops before userspace.
