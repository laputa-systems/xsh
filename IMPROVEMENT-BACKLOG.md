...
- **A01 · P1.** Continue the native/Rust ownership audit in the remaining runtime groups. `tests/runtime/modules.rs` now owns ELF, network, archive, raw-byte, and helper-process fixtures; its language-only module checks live in `tests/xsh/stdlib/module.xsh` and `tests/xsh/run.xsh`. `tests/runtime/run.rs` keeps `/dev/stdin`, CLI, trace, and process boundaries; its float/JSON value check is native. Move any remaining assertion-only cases to disk-backed native tests without replacing host fixtures.
