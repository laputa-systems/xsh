...
- **B11 · P1.** Finish the final macOS rerun and feature/profile gates, then close the migration. `results-b11-cumulative-macos.json` passes all 24 applicable workloads in three rounds with exact parity before the Linux reader cleanup. `results-b11-cumulative-linux.json` passes all 30 pinned Linux workloads in three rounds with exact parity after the cleanup. The earlier full run exposed `text_wrap_unicode`; its `Str.wrap` and `Str.fields` bindings returned to native, and `bench/stdlib-port/results-b11-text-wrap.json` records passing fixed workloads on both hosts. The first Linux cumulative round exposed `linux.meminfo`, `system.memory`, and `system.os_release`; `results-b11-linux-text-{prechange,native}.json` records three failing embedded and three passing native B0 workloads with exact parity. Their unused embedded sources and append bridge were removed. Repeat copied-product, native corpus, and API checks after final builds.
