...
- **A01 · P1.** Continue the native/Rust ownership audit. The stream module, `fs.walk` values, explicit `main()` invocation, loaded-module contracts, and collection updates now have native owners; the redundant `modules-basic.xsh` Rust wrapper was removed after checking those behaviors against existing native tests. Move remaining language assertions to disk-backed native tests when the host harness adds no boundary.
