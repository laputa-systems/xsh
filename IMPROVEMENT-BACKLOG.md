...
- **A01 · P1.** Continue the native/Rust ownership audit beyond the completed stream module, the `fs.walk` value checks in `tests/xsh/stdlib/fs.xsh`, and explicit `main()` invocation now in `tests/xsh/run.xsh`; move language assertions to disk-backed native tests when the host harness adds no boundary.
