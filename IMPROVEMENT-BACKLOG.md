...
- **J11 · P2.** Migrate the x86_64 installer image builder from removed `pm install` after native package execution supports that target. `../packages` now writes and reads a target-specific x86_64 BuildPlan; the 24-node installer selection produced byte-identical plans in two runs, and the local PM suite passed 117 cases with two host skips. `pm repo build` explicitly rejects x86_64 before store mutation. Next port native x86_64 execution, artifact receipts, root and generation validation, and the Docker runner; prove build and root composition before changing the installer. Preserve its installer and target roots, then run its image and QEMU tests; the published x86_64 kernel currently stops before userspace.
