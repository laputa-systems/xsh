...
proc test_rg_reports_matches_with_line_numbers(ctx: TestContext) [fs, process, error] {
  let file = test.temp_file(ctx, name: "notes.txt", contents: b"alpha\nbeta\nalphabet\n")?
  let output = run.text xsh_bin() rg.xsh -- -n alpha $file ?
  test.contains(output, "1:alpha")?
  test.contains(output, "3:alphabet")?
}

proc test_rg_count_and_filename(ctx: TestContext) [fs, process, error] {
  let left = test.temp_file(ctx, name: "left.txt", contents: b"needle\n")?
  let right = test.temp_file(ctx, name: "right.txt", contents: b"needle\n")?
  let no_filename = run.text xsh_bin() rg.xsh -- -h needle $left $right ?
...
""",
  ...
  let count = run.text xsh_bin() rg.xsh -- -c needle $left ?
  test.eq(count.trim(), "1")?
  let named_counts = run.text xsh_bin() rg.xsh -- -c needle $left $right ?
  test.contains(named_counts, f"${left}:1")?
  test.contains(named_counts, f"${right}:1")?
}

proc test_rg_word_line_pattern_and_globs(ctx: TestContext) [fs, process, error] {
...
""")?
  ...
  let word = run.text xsh_bin() rg.xsh -- -w alpha $keep ?
...
""",
  ...
  let line = run.text xsh_bin() rg.xsh -- -x needle $keep ?
...
""",
  ...
  let fixed_case = run.text xsh_bin() rg.xsh -- -F -i needle $keep ?
