##! Embedded implementation of the public `linux.meminfo` text-backed entry.
#
...
#
# The script-backed entry replaces the native body, and with it the dispatch
# that carried the gate, the canned values, and the log append, so this module
# reproduces all three. Without `XSH_LINUX_DRY_RUN` or `XSH_LINUX_REAL` it
# fails with `linux-unimplemented` before any file is opened. With
# `XSH_LINUX_DRY_RUN` set, it reports its canned value and appends one
...
# `Err`, exactly as the native entry reports it. The path is fixed at
# `/proc/meminfo`. No environment variable, parameter, or other switch changes
# which file is read.

# The error kind the entry below reports.
#
...
# `linux-unimplemented`, `linux-meminfo`, and `linux-dry-run-log` visible to
# callers.
...
pure parse_field_int(text: Str) -> Int? {
  ...
  if first == 48 and end == 1 {
    return 0
  }
  # A nonzero first digit cannot spell a signed or zero-padded field. The
  # built-in decimal parser has the baseline's grammar for this common case,
  # including its overflow and invalid-character failures.
  if first >= 49 and first <= 57 {
