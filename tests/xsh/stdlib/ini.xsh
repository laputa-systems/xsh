...
proc test_ini_encode_orders_globals_then_sections()  [fs, error] {
  ...
  test.eq(ini.encode({b: "2", a: "1"})?, "a = 1\nb = 2\n")?

  # Only one section: its header, then its keys in key order. An empty section
  # is its header alone.
  test.eq(ini.encode({server: {host: "example.test"}})?, "[server]\nhost = example.test\n")?
  test.eq(ini.encode({s: {}})?, "[s]\n")?
  ...
  test.eq(ini.encode({s2: {b: "2"}, s1: {a: "1"}})?, "[s1]\na = 1\n\n[s2]\nb = 2\n")?
  test.eq(ini.encode({g: "1", s1: {}, s2: {}})?, "g = 1\n\n[s1]\n\n[s2]\n")?
...
proc test_ini_encode_normalizes_section_keys()  [fs, error] {
  ...
  test.eq(ini.encode({s: {Host: "1", host: "2"}})?, "[s]\nhost = 2\n")?
  test.eq(ini.encode({s: {host: "2", Host: "1"}})?, "[s]\nhost = 2\n")?
  test.eq(ini.encode({s: {Host: "1", host: "2", HOST: "3"}})?, "[s]\nhost = 2\n")?
  test.eq(ini.encode({s: {"_A": "1", "_a": "2"}})?, "[s]\n_a = 2\n")?
  ...
  test.eq(ini.encode({s: {A: "1", B: "2", a: "3"}})?, "[s]\na = 3\nb = 2\n")?
  test.eq(ini.encode({s: {Z: "1", _a: "2"}})?, "[s]\n_a = 2\nz = 1\n")?
...
proc test_ini_encode_rejects_invalid_names()  [fs, error] {
  ...
  test.error_kind(ini.encode({s: {"a]b": "1"}}), "ini-key")?
  test.error_kind(ini.encode({s: {"a\nb": "1"}}), "ini-key")?
...
proc test_ini_encode_writes_multiline_values()  [fs, error] {
  ...
  test.eq(ini.encode({s: {message: "hello\nworld"}})?, "[s]\nmessage = hello\n  world\n")?
  test.eq(ini.encode({a: "x\n\ny"})?, "a = x\n  \n  y\n")?
  ...
  test.eq(ini.encode({a: "x\n"})?, "a = x\n  \n")?
  test.eq(ini.encode({a: ""})?, "a = \n")?

  # The last line ends with exactly one newline, and the result re-decodes to
  # the value it was built from.
  test.eq(ini.encode({a: "1"})?, "a = 1\n")?
  test.eq(ini.encode({a: "1"})?.ends_with("\n\n"), false)?
  let round_trip = ini.decode(ini.encode({global: "root", server: {message: "hello\nworld"}})?)?
  test.eq(round_trip.global, "root")?
  test.eq(round_trip.server.message, "hello\nworld")?
}
