...
#[test]
fn nested_bindings_shadow_and_restore_the_enclosing_scope() {
    let output = xsh(["tests/fixtures/runtime/lexical-shadowing.xsh"]);
    ...
    // A nested declaration binds its own slot: the inner reads see the inner
    // value, an inner assignment changes only the inner binding, and the outer
    // binding is back once the inner scope ends.
    assert_eq!(
        String::from_utf8(output.stdout).unwrap(),
        "ab;cd;\none;\na;\ninner\nouter\n200\nab|outer\n|outer\n"
    );
