...
// Rebuild for every extension lookup: the host overlay is observed at each
// call, including each suffix that lookup_path tries. Later rows replace
// earlier entries for the same normalized extension.
