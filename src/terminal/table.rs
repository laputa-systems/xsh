...
pub(crate) fn terminal_table_width_for_stdout(min_width: usize, default_width: usize) -> usize {
    terminal_table_width(StdFd::Stdout, min_width, default_width)
}

#[cfg(feature = "tools")]
pub(crate) fn terminal_table_width_for_stderr(min_width: usize, default_width: usize) -> usize {
    terminal_table_width(StdFd::Stderr, min_width, default_width)
}

fn terminal_table_width(fd: StdFd, min_width: usize, default_width: usize) -> usize {
    terminal_columns(fd)
...
#[cfg(unix)]
fn terminal_columns(fd: StdFd) -> Option<usize> {
    let size = match fd {
        StdFd::Stdout => termios::tcgetwinsize(rstdio::stdout()).ok()?,
        #[cfg(feature = "tools")]
        StdFd::Stderr => termios::tcgetwinsize(rstdio::stderr()).ok()?,
    };
    if size.ws_col == 0 {
        None
    } else {
...
#[cfg(not(unix))]
fn terminal_columns(_fd: StdFd) -> Option<usize> {
    None
}
