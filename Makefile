...
PERF_PROFILE ?= profiling
PERF_PROFILE_DIR = target/$(PERF_PROFILE)
DIST_PROFILE ?= dist
...
perf-linux:
	docker build -t xsh-test -f Dockerfile.test .
	docker run --rm -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test sh -c "cargo build --profile $(PERF_PROFILE) --features perf-metrics --bin xsh && cargo build --profile $(PERF_PROFILE) -p xsht"
	docker run --rm --cap-add SYS_PTRACE --security-opt seccomp=unconfined -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test $(PERF_PROFILE_DIR)/xsh perf/run.xsh -- --scenario extension-count --scale $${XSH_PERF_SCALE:-8} --syscalls --no-build --xsh $(PERF_PROFILE_DIR)/xsh --xsht $(PERF_PROFILE_DIR)/xsht

perf-linux-extension-count:
	docker build -t xsh-test -f Dockerfile.test .
	docker run --rm -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test sh -c "cargo build --profile $(PERF_PROFILE) --features perf-metrics --bin xsh && cargo build --profile $(PERF_PROFILE) -p xsht"
	docker run --rm --cap-add SYS_PTRACE --security-opt seccomp=unconfined -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test $(PERF_PROFILE_DIR)/xsh perf/compare-extension-count.xsh -- --syscalls --no-build --xsh $(PERF_PROFILE_DIR)/xsh --xsht $(PERF_PROFILE_DIR)/xsht

perf-linux-flamegraph:
	docker build -t xsh-test -f Dockerfile.test .
	docker run --rm --privileged --cap-add SYS_ADMIN --cap-add SYS_PTRACE --security-opt seccomp=unconfined -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test sh -c "cargo build --profile $(PERF_PROFILE) --features perf-metrics --bin xsh && mkdir -p target/perf && rm -f target/perf/extension-count.* && $(PERF_PROFILE_DIR)/xsh perf/make-corpus.xsh -- --root /work/target/perf/corpus --scale $${XSH_PERF_SCALE:-8} && perf record -F $${XSH_PERF_FREQ:-999} -g -o target/perf/extension-count.perf.data -- $(PERF_PROFILE_DIR)/xsh perf/scenarios/extension-count.xsh -- /work/target/perf/corpus && perf script --demangle -i target/perf/extension-count.perf.data > target/perf/extension-count.perf.script && $(PERF_PROFILE_DIR)/xsh showcase/perf-collapse.xsh -- target/perf/extension-count.perf.script > target/perf/extension-count.folded && $(PERF_PROFILE_DIR)/xsh showcase/flamegraph.xsh -- target/perf/extension-count.folded > target/perf/extension-count.svg && $(PERF_PROFILE_DIR)/xsh showcase/perf-collapse.xsh -- --top 20 target/perf/extension-count.perf.script > target/perf/extension-count.top && head -20 target/perf/extension-count.top"
...
perf-linux-showcases:
	docker build -t xsh-test -f Dockerfile.test .
	docker run --rm --cap-add SYS_PTRACE --security-opt seccomp=unconfined -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test sh -c "cargo build --profile $(PERF_PROFILE) --features perf-metrics --bin xsh && cargo build --profile $(PERF_PROFILE) -p xsht && $(PERF_PROFILE_DIR)/xsh perf/showcase-tests.xsh -- --showcase $${SHOWCASE:-all} --syscalls --repeat $${XSH_PERF_REPEAT:-1} --min-duration-ms $${XSH_PERF_MIN_DURATION_MS:-0} --no-build --xsh $(PERF_PROFILE_DIR)/xsh --xsht $(PERF_PROFILE_DIR)/xsht"
...
perf-linux-showcase-flamegraphs:
	docker build -t xsh-test -f Dockerfile.test .
	docker run --rm --privileged --cap-add SYS_ADMIN --cap-add SYS_PTRACE --security-opt seccomp=unconfined -v $(CURDIR):/work -v $(CURDIR)/target/aarch64-unknown-linux-musl:/work/target -v xsh-cargo-registry:/root/.cargo/registry -w /work xsh-test sh -c "cargo build --profile $(PERF_PROFILE) --features perf-metrics --bin xsh && cargo build --profile $(PERF_PROFILE) -p xsht && $(PERF_PROFILE_DIR)/xsh perf/showcase-tests.xsh -- --showcase $${SHOWCASE:-all} --syscalls --flamegraphs --repeat $${XSH_PERF_REPEAT:-1} --min-duration-ms $${XSH_PERF_MIN_DURATION_MS:-1000} --no-build --freq $${XSH_PERF_FREQ:-999} --xsh $(PERF_PROFILE_DIR)/xsh --xsht $(PERF_PROFILE_DIR)/xsht"
	docker run --rm -v $(CURDIR):/host alpine:3.21 sh -c "mkdir -p /host/target/perf && cp -R /host/target/aarch64-unknown-linux-musl/perf/showcase-tests-* /host/target/perf/ 2>/dev/null || true"
