...
| Privileged Linux mount behavior | In the pinned privileged `xsh-test` image: `cargo test -p xsh --target aarch64-unknown-linux-musl --test linux_priv --features linux-priv-tests linux_priv_tmpfs_mount_is_mountpoint_disk_usage_and_cleanup -- --exact --nocapture` | Run the full `linux_priv` test binary in the same image with the target and feature flags |
