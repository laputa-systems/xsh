...
| system-report relationships, source parsers, CPU list parsing, section projection, JSON conversion/redaction, terminal-safe rendering, mount usage, cgroup accounting, firmware issues, and offline CLI replay | `target/debug/xsht test --exact --jobs 1 tests/xsh/system-report.xsh::test_system_report_storage_parses_mountinfo_escapes_and_stacked_mounts` | `target/debug/xsht test --jobs 1 tests/xsh/system-report.xsh` |
| system-report coverage denominator and syscall trace parsing | `target/debug/xsht test --exact --jobs 1 dev/tests/test-system-report-check.xsh::test_system_report_coverage_manifest_contract` | `xsh dev system-report-check` validates the checked-in manifest; `xsh dev system-report-check --no-subprocess --xsh-bin ABSOLUTE_XSH --script ABSOLUTE_SYSTEM_REPORT` traces the production path |
| Raw host path, argv, environment bytes, bounded rooted reads, rooted directory enumeration, and rooted filesystem counters | `target/debug/xsht test --exact --jobs 1 tests/xsh/stdlib/fs.xsh::test_fs_root_operations_reject_traversal` and `cargo test --test integration runtime::modules::helper_binaries_cover_raw_argv_env_path_and_glob_boundaries -- --exact` | Native stdlib gate plus the filtered runtime gate; the filesystem-name case skips on macOS because the host returns `EILSEQ` |
