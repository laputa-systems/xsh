...
proc main(target: Str, ...bins: List[Str]) [process, error, io] {
  ...
  let expected_machine = if "aarch64" in target { "AArch64" } else { "Advanced Micro Devices X86-64" }

  for bin in bins {
    let candidate = fp"${bin_dir}/${bin}"
    let binary_data = candidate.read_bytes()?
    if binary_data.len() < 4 or binary_data.slice(0, length: 4) != b"\x7fELF" {
    ...
    if ! header.stdout.contains(expected_machine) {
