...
fn stream_doc(stage: &str) -> ReferenceDoc {
    let (summary, contract, tags, signature): (
        ...
        "fold" => ("Reduces stream items with an explicit accumulator block.", "Accumulator updates follow source order for the serial form; parallel variants require an associative reduction contract. The initial value fixes the accumulator type.", &["stream", "reduction", "accumulator"], "fold(init, block) -> A"),
        "reduce" => ("Reduces stream items with an explicit accumulator block.", "Accumulator updates follow source order for the serial form; parallel variants require an associative reduction contract. The initial value fixes the accumulator type.", &["stream", "reduction", "accumulator"], "reduce(init, block) -> A"),
