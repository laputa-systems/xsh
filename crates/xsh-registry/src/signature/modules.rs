...
fn tui_module() -> ModuleSig {
    // Escape-sequence producers execute as embedded XSH. Padding keeps its
    // native width scan; `read_secret` owns terminal I/O and stays native.
    let escapes = [
...
pub fn record_docs(name: &str) -> ApiDocs {
    let doc =
        record_doc(name).unwrap_or_else(|| panic!("missing record documentation for '{name}'"));
    ApiDocs {
...
fn record_doc(name: &str) -> Option<RecordDoc> {
    let doc: (&'static str, &'static str, &'static [&'static str]) = match name {
        "ArchiveEntry" => (
            "Describes one archive member.",
            "The path is an archive-relative member name; it is not an unrestricted host path.",
            &["archive", "path", "record"],
        ),
        "DiffResult" => (
            "Summarizes a generated unified diff.",
            "The counts describe the returned diff text and do not imply that a patch was applied.",
            &["diff", "text", "record"],
        ),
        "DnsHost" => (
            "Describes one resolved DNS host address.",
            "The address is resolver output at lookup time and may become stale immediately.",
            &["dns", "net", "host-state"],
        ),
        "DnsLookup" => (
            "Describes one DNS record answer.",
            "Record type, value, and TTL come from the resolver boundary and are not independently authenticated by the record.",
            &["dns", "net", "record"],
        ),
        "ElfDynamicTag" => (
            "Describes one ELF dynamic-section tag.",
            "The tag is parsed metadata from an input file; inspecting it does not load or execute the object.",
            &["elf", "binary", "record"],
        ),
        "ElfInfo" => (
            "Describes ELF headers and dynamic dependencies.",
            "Fields reflect the inspected bytes and malformed binaries remain errors before this record is returned.",
            &["elf", "binary", "record"],
        ),
        "EnvEntry" => (
        ...
        "EnvPathEntry" => (
            ...
            "An empty component is retained as data because it can have path-search meaning.",
            &["env", "path", "record"],
        ),
        "FsCopyTreeResult" => (
            "Reports files and directories copied by a tree operation.",
            "Counts describe completed host operations and do not by themselves guarantee a complete source snapshot.",
        ...
        "LinuxBlockDevice" => (
            "Describes one Linux block device.",
            "Device metadata is host-global and may require privilege to inspect or act upon.",
            &["linux", "device", "record"],
        ),
        "LinuxDiskUsage" => (
            "Reports Linux filesystem disk usage.",
            "The values are a point-in-time kernel/filesystem observation rather than a reservation.",
            &["linux", "filesystem", "record"],
        ),
        "LinuxFileAttrs" => (
            "Describes Linux extended file attributes.",
            "Attributes belong to host filesystem state and may require privilege to read or change.",
            &["linux", "filesystem", "record"],
        ),
        "LinuxInterface" => (
            "Describes one Linux network interface.",
            "The record is a host-global networking snapshot and can become stale before use.",
            &["linux", "network", "record"],
        ),
        "LinuxInterfaceAddress" => (
            "Describes one address assigned to a Linux interface.",
            "The address record is observational; configuring it requires the separate privileged API.",
            &["linux", "network", "record"],
        ),
        "LinuxRoute" => (
        ...
        "LinuxLoopDevice" => (
            ...
            "An attached loop resource remains host-owned until an explicit detach operation releases it.",
            &["linux", "loop", "ownership"],
        ),
        "LinuxMemInfo" => (
            "Reports Linux memory counters.",
            "Counters are a point-in-time kernel observation and can change between fields or calls.",
            &["linux", "memory", "host-state"],
        ),
        "LinuxBlkid" => (
            "Describes Linux block-device identification data.",
            "Identification fields come from the device probe and must not be treated as a write authorization.",
            &["linux", "device", "inspection"],
        ),
        "LinuxFsck" => (
            "Reports a Linux filesystem check result.",
        ...
        "LinuxModule" => (
            "Describes one loaded Linux kernel module.",
            "The record reflects host kernel state at query time and may disappear before a later operation.",
            &["linux", "kernel", "host-state"],
        ),
        "LinuxModuleParam" => (
            "Describes one Linux kernel-module parameter.",
            "Parameter values are host metadata and changing them requires an explicit privileged operation.",
            &["linux", "kernel", "configuration"],
        ),
        "LinuxOpenFile" => (
            "Describes one open file held by a Linux process.",
            "The record is a point-in-time process snapshot and does not keep the file descriptor alive.",
            &["linux", "process", "filesystem"],
        ),
        "LinuxPartition" => (
            "Describes one Linux partition.",
            "Partition data is storage metadata; changing it requires a separate explicit destructive operation.",
            &["linux", "storage", "record"],
        ),
        "LinuxPartitionTable" => (
            "Describes a Linux partition table.",
            "The record is a complete inspection snapshot and is not itself a request to write storage.",
            &["linux", "storage", "record"],
        ),
        "LinuxRfkill" => (
            "Describes Linux radio-block state.",
            "The state is host device policy and can change outside the current script.",
            &["linux", "rfkill", "host-state"],
        ),
        "LinuxUevent" => (
            "Describes one Linux device uevent.",
        ...
        "MeasuredCommand" => (
            ...
            &["time", "process", "status-data"],
        ),
        "MimeInfo" => (
            "Describes a MIME type lookup result.",
            "The record is derived from a media-type database or path spelling and does not verify file contents.",
            &["mime", "lookup", "record"],
        ),
        "MimeParse" => (
            "Describes parsed media-type components.",
            "Fields reflect validated media-type syntax; malformed input never becomes a partial trusted record.",
            &["mime", "parsing", "record"],
        ),
        "NetHeader" => (
            "Describes one HTTP header.",
            "Header names and values remain protocol text; callers must apply any application-specific trust or duplicate policy.",
            &["net", "http", "record"],
        ),
        "NetPool" => (
            "Represents an evaluator-owned HTTP connection pool.",
            "Pool ownership and cleanup stay with the evaluator session; close it explicitly when the workflow ends.",
            &["net", "http", "ownership"],
        ),
        "NetResponse" => (
        ...
        "PatchResult" => (
            "Reports files changed by a rooted patch.",
            "The result describes a patch applied below the supplied root; it is not an authorization to write outside that root.",
            &["patch", "rooted", "record"],
        ),
        "ProcessEntry" => (
            "Describes one process-table entry.",
            "The record is a point-in-time host snapshot and the process may exit before any later action.",
            &["process", "inspection", "record"],
        ),
        "ProcessPort" => (
            "Describes one process-owned network port.",
            "Listener data is observational and can change independently of the record after enumeration.",
        ...
        "Signal" => (
            ...
            "The numeric signal is host protocol data; sending it is a separate explicit process operation.",
            &["process", "signal", "record"],
        ),
        "Spawn" => (
            "Represents an owned spawned process.",
            "The handle lifecycle must end through wait, cancel, detach, or transfer; dropping ownership has cleanup semantics.",
        ...
        "SystemOsRelease" => (
            ...
            "Fields come from host release files and missing metadata remains an error rather than an invented value.",
            &["system", "os", "record"],
        ),
        "TestCall" => (
            "Describes one recorded native-test mock call.",
            "The call record belongs to the mock scope and is evidence for assertions, not a live host handle.",
            &["test", "mock", "record"],
        ),
        "TestContext" => (
            "Carries native-test resources and assertion context.",
            "The context is owned by one native test invocation and must not escape that test's lifecycle.",
            &["test", "native-tests", "ownership"],
        ),
        "Uname" => (
            "Describes the host kernel identity returned by uname.",
            "The fields identify the running host at query time and are not a portable feature-negotiation contract.",
            &["system", "uname", "record"],
        ),
        "UnixChildEvent" => (
            "Describes one reaped Unix child event.",
            "Reaping consumes host wait state; the event must be correlated with the process owner that spawned the child.",
            &["unix", "process", "reaping"],
        ),
        "UnixGroupId" => (
            "Describes Unix group identity numbers.",
            "The numeric IDs are host account metadata and do not themselves alter process credentials.",
            &["unix", "identity", "group"],
        ),
        "UnixId" => (
            "Describes Unix user and group identity numbers.",
            "The record reports host credentials; changing them requires a separate explicit process operation.",
            &["unix", "identity", "record"],
        ),
        "UnixKillAllResult" => (
            "Reports a Unix process-set signal operation.",
            "The result records attempted host effects and does not imply every selected process accepted the signal.",
            &["unix", "process", "signal", "status-data"],
        ),
        "UnixLoggedProcessGroup" => (
            "Represents an owned Unix process group with logging.",
            "The group and log ownership must be closed through the documented lifecycle before the supervising scope ends.",
            &["unix", "process", "ownership", "logging"],
        ),
        "UnixPid1Event" => (
            "Describes one PID 1 lifecycle event.",
            "PID 1 events are host process-lifecycle state and are meaningful only in the corresponding Unix supervisor boundary.",
            &["unix", "pid1", "process"],
        ),
        "UnixPid1Shutdown" => (
            "Describes a requested PID 1 shutdown action.",
            "Shutdown data represents an external host-global transition and must be handled as final control flow.",
            &["unix", "pid1", "shutdown"],
        ),
        "UnixSpawnedChild" => (
            "Represents a Unix child spawned in a process group.",
            "The child handle owns cleanup and wait coordination until the caller explicitly transfers or ends it.",
            &["unix", "process", "ownership"],
        ),
        "UnixTtyAttrs" => (
            "Describes Unix terminal attributes.",
            "Attributes are tied to a host terminal descriptor and restoring them remains the caller's responsibility after mutation.",
            &["unix", "tty", "record"],
        ),
        "User" => (
            "Describes a Unix user account.",
            "The record is a host identity snapshot and does not itself grant the script that user's privileges.",
