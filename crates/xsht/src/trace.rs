...
impl SyscallSummary {
    ...
    pub fn attribute_operations(&mut self, records: &[SyscallTraceRecord], trace_jsonl: &str) {
        ...
        for line in trace_jsonl.lines() {
            let Ok(value) = xsh::host::json::parse_raw_json(line) else {
                continue;
            };
            let kind_value = xsh::host::json::raw_json_get(&value, "kind");
            let kind = match kind_value.and_then(xsh::host::json::raw_json_as_str) {
                Some(kind)
                    if matches!(
                        kind,
                        "module.result"
            ...
            let start_us = match xsh::host::json::raw_json_get(&value, "start_time_us")
                .and_then(xsh::host::json::raw_json_as_u64)
            {
                Some(v) if v > 0 => v,
                _ => continue,
            };
            let duration_us = match xsh::host::json::raw_json_get(&value, "duration_us")
                .and_then(xsh::host::json::raw_json_as_u64)
            {
                Some(v) => v,
                _ => continue,
            };
            let name = xsh::host::json::raw_json_get(&value, "name")
                .and_then(xsh::host::json::raw_json_as_str)
                .or_else(|| {
                    xsh::host::json::raw_json_get(&value, "payload")
                        .and_then(|payload| xsh::host::json::raw_json_get(payload, "target"))
                        .and_then(|target| xsh::host::json::raw_json_get(target, "display"))
                        .and_then(xsh::host::json::raw_json_as_str)
                })
                .unwrap_or("?")
                .to_string();
...
impl SyscallSummaryRenderer {
    ...
    pub fn render_jsonl(&self, summary: &SyscallSummary) -> String {
        ...
        let mut output = xsh::host::json::compact_raw_json(&syscall_summary_json_value(dto));
        output.push('\n');
        output
    }
}
...
impl TraceJsonlRenderer {
    ...
    fn render_event(&self, event: &TraceEvent, sources: &SourceMap, output: &mut String) {
        ...
        output.push_str(&xsh::host::json::compact_raw_json(&trace_event_json_value(
            dto,
        )));
    }
}

/// Renders the reduced JSONL event shape consumed by `xsht test --coverage`.
///
/// The full trace JSONL renderer is intended for users of `xsht trace`; this
/// coverage format deliberately contains only source metadata and spans so
/// coverage collection is independent of runtime payload representation.
#[derive(Clone, Debug, Default)]
pub struct CoverageTraceRenderer;

impl CoverageTraceRenderer {
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn render_events(&self, events: &[TraceEvent], sources: &SourceMap) -> String {
        ...
        for file in sources.files() {
            let value = raw_json_object([
                ("kind".to_string(), raw_json_string("source.file")),
                ("file".to_string(), raw_json_string(file.name())),
                ("line_count".to_string(), raw_json_usize(file.line_count())),
            ]);
            output.push_str(&xsh::host::json::compact_raw_json(&value));
            output.push('\n');
        }
        for event in events {
            let mut fields = vec![("kind".to_string(), raw_json_string(event.kind.as_str()))];
            if let Some(api_id) = &event.api_id {
                fields.push(("api_id".to_string(), raw_json_string(api_id)));
            }
            if let Some(name) = &event.name {
                fields.push(("name".to_string(), raw_json_string(name)));
            }
            if let Some(span) = event.source_span {
                append_coverage_span(&mut fields, "source_span", span, sources);
            }
            if let Some(span) = event.definition_span {
                append_coverage_span(&mut fields, "definition_span", span, sources);
            }
...
fn append_coverage_span(
    ...
    fields.push((
        key.to_string(),
        raw_json_object([
            ("file".to_string(), raw_json_string(start.file)),
            ("start_line".to_string(), raw_json_usize(start.line)),
            ("end_line".to_string(), raw_json_usize(end.line)),
            ("start_offset".to_string(), raw_json_usize(span.start())),
            ("end_offset".to_string(), raw_json_usize(span.end())),
        ]),
    ));
}
...
impl TraceSummaryRenderer {
    ...
    pub fn render_jsonl(&self, events: &[TraceEvent], sources: &SourceMap) -> String {
        ...
        let mut output = xsh::host::json::compact_raw_json(&trace_summary_json_value(dto));
        output.push('\n');
        output
    }
}
...
fn normalize_payload(mut payload: TracePayload) -> TracePayload {
    match &mut payload {
        TracePayload::RunEnd { pid, .. }
        | TracePayload::SpawnReady { pid, .. }
        | TracePayload::WaitEnd { pid, .. }
        | TracePayload::SpawnCancel { pid, .. }
        | TracePayload::PipelineSegmentEnd { pid, .. } => {
            *pid = None;
        }
        _ => {}
    }
    ...
    pub fn normalize_events(&self, events: &[TraceEvent]) -> Vec<TraceEvent> {
        ...
        events
            ...
            .map(|(index, event)| {
                ...
                normalized.payload = normalize_payload(normalized.payload);
                normalized
            })
            .collect()
    }
...
#[cfg(test)]
mod tests {
    use super::super::table::table_text_width;
    use super::{
        CoverageTraceRenderer, SyscallSummary, SyscallSummaryRenderer, SyscallTraceRecord,
        TraceArg, TraceError, TraceEvent, TraceJsonlRenderer, TraceKind, TraceNormalizer,
        TracePayload, TraceStatus, TraceSummaryRenderer, TraceTextRenderer, TraceTiming,
    };
    use xsh::frontend::source::{SourceMap, Span};
    use xsh::trace::model::{
    ...
    #[test]
    fn renders_coverage_trace_with_source_metadata_and_spans() {
        let mut sources = SourceMap::new();
        let id = sources.add_file("sample.xsh", "proc main() {\n  return\n}\n");
        let event = TraceEvent::new(1, TraceKind::ProcEnter)
            .with_name("main")
            .with_span(Span::new(id, 0, 5))
            .with_definition_span(Span::new(id, 0, 16));

        let rendered = CoverageTraceRenderer::new().render_events(&[event], &sources);
        let values = rendered
            .lines()
            .map(|line| {
                miniserde::json::from_str::<miniserde::json::Value>(line)
                    .expect("parse coverage JSON")
            })
            .collect::<Vec<_>>();

        assert_eq!(json_str(json_field(&values[0], "kind")), "source.file");
        assert_eq!(json_str(json_field(&values[1], "kind")), "proc.enter");
        assert_eq!(
            json_str(json_field(json_field(&values[1], "source_span"), "file")),
            "sample.xsh"
        );
        assert_eq!(
            json_u64(json_field(
                json_field(&values[1], "definition_span"),
                "start_line"
            )),
            1
        );
